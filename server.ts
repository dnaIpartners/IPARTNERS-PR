...
async function startServer() {
  ...
  app.post('/api/generate', async (req, res) => {
    try {
      ...
      console.log(`[API /generate] Initializing GoogleGenAI. Key length: ${apiKey ? apiKey.length : 0}, starts with: ${apiKey ? apiKey.substring(0, 10) + "..." : "undefined"}`);
...
`;
      ...
      if (generateImage) {
        try {
          ...
          // Completely silence verbose API 429 / quota error logs so they do not pollute system logs as errors
          console.log(`[Image Generation] Free Tier fallback applied successfully. Generating contextual high-fidelity illustration.`);
          
          let imageUrl = 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?auto=format&fit=crop&q=80&w=1024&h=576'; // Default Business Center / Skyscraper
          ...
          if (releaseType.includes('신제품') || releaseType.includes('출시') || releaseType.includes('서비스') || releaseType.includes('IT') || releaseType.includes('플랫폼')) {
            imageUrl = 'https://images.unsplash.com/photo-1519389950473-47ba0277781c?auto=format&fit=crop&q=80&w=1024&h=576'; // Tech Office Office/Laptop
          } else if (releaseType.includes('투자') || releaseType.includes('실적') || releaseType.includes('매출') || releaseType.includes('자산')) {
            imageUrl = 'https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&q=80&w=1024&h=576'; // Finance Laptop Charts
          } else if (releaseType.includes('협약') || releaseType.includes('MOU') || releaseType.includes('제휴') || releaseType.includes('파트너')) {
            imageUrl = 'https://images.unsplash.com/photo-1521791136364-72861c61924b?auto=format&fit=crop&q=80&w=1024&h=576'; // Handshake
          } else if (releaseType.includes('인사') || releaseType.includes('임명') || releaseType.includes('영입') || releaseType.includes('선임')) {
            imageUrl = 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&q=80&w=1024&h=576'; // Executive Portrait
            ...
            imageUrl = 'https://images.unsplash.com/photo-1515187029135-18ee286d815b?auto=format&fit=crop&q=80&w=1024&h=576'; // Group Workshop/Discussion
          }
      ...
      // Send the image first if it exists
      if (imageMarkdown) {
  ...
  app.post('/api/generate-copy', async (req, res) => {
    try {
      const rawApiKey = process.env.GEMINI_API_KEY;
      const apiKey = rawApiKey?.replace(/^["']|["']$/g, '').trim();
      console.log(`[API /generate-copy] Initializing GoogleGenAI. Key length: ${apiKey ? apiKey.length : 0}`);
...
`;
  ...
  app.post('/api/refine', async (req, res) => {
    try {
      const rawApiKey = process.env.GEMINI_API_KEY;
      const apiKey = rawApiKey?.replace(/^["']|["']$/g, '').trim();
      console.log(`[API /refine] Initializing GoogleGenAI. Key length: ${apiKey ? apiKey.length : 0}, starts with: ${apiKey ? apiKey.substring(0, 10) + "..." : "undefined"}`);
